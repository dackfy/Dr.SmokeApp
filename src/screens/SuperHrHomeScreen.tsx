...
export default function SuperHrHomeScreen({
  session,
  onLogout,
  onRefreshSession,
  onGoProfile,
}: SuperHrHomeScreenProps) {
  ...
  const loadScheduleRegions = React.useCallback(async (sourceRegions?: RegionCard[]) => {
    ...
    try {
      ...
      if (res.status === 404 || res.status === 501) {
        const fallbackScheduleRegions = buildFallbackScheduleRegionCards(sourceRegions ?? regionCards)
  ...
  }, [loadRegions, loadScheduleRegions])
                    ...
                    if (day.dateKey) {
                      ...
                      if (selectedScheduleMonthStateKey) {
                        setSelectedScheduleDateByMonth(current =>
                          current[selectedScheduleMonthStateKey] === day.dateKey
                            ? current
                            : { ...current, [selectedScheduleMonthStateKey]: day.dateKey },
                        )
                      }
